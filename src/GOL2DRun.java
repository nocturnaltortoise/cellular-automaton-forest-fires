...
import ca_grids.GOL2DGrid;                  
import ca_cells.GOL2DCell;
import graphics.ImageAxB;


public class GOL2DRun extends JPanel{        // The code must extend Jpanel in the declaration
  ...
  public static void main(String[] argv) {   // Standard main (with command line input)
    ...
    // java GOL2DRun (1)(int)xDim (2)(int) yDim (3)(int) % for random initialisation (4)(int) number of generations
    // java GOL2DRun (1)(char[0]) = 'r' or 'R'-triggers read based initialisation (2)(int) number of generations
    //***************************************************************************************************************
    ...
    if ((argv.length >=2)||(argv.length<=5)) {      // Check for correct number of command line arguments
      ...
      System.out.println("Input Parameters Help:\n\n two way to run: -\n\n(A)******** java GOL2DRun (int){1} (int){2} (int){3} (int){4}\n\n");
      System.out.println("1 -x dimension of array\n2 -y dimension of array\n3 -Percent set initialisation\n4 -No. of generations\n\n");
      System.out.println("(B)******** java GOL2DRun (char){1} (int){2}\n\n1 -\'r\' or \'R\',read \"GOL_input_state_array.txt\" infile for initialisation.\n2 -No. of generations.");
      System.exit(1);
    }
    ...
    if(doRead==true) {                                // if "do read" command line input is used
      System.out.println("Attempting to obtain cell array size from file \"GOL_input_state_array.txt\"");
      try {                                           // use try-catch exception handling
        BufferedReader inarray = new BufferedReader(new FileReader("GOL_input_state_array.txt")); // open the input file as a BufferedReader
    ...
    GOL2DGrid cellSet = new GOL2DGrid(xSizeIn,ySizeIn);  // call constructor for the GOL2DGrid class object now we know dimensions
    ...
    if (doRead==true){                                // if "do read" command line input is used
      try {                                           // use try-catch exception handling
        BufferedReader inarray = new BufferedReader(new FileReader("GOL_input_state_array.txt"));  // open the input file again
        String strIn;                                 // a string to hold a line of text input
        int linecount=0;                              // a line count to set the ySizeIn from the number of lines in the input text file 
        while ((strIn = inarray.readLine()) != null) {                // While the file can still be read (cos it's not reached the end)...
    ...
    frame.getContentPane().add(new GOL2DRun());                           // tell the frame to add the contentPane (this actions the
    ...
    try {                                           // try-catch Exception handling to open an output textfile
      PrintStream outtext = new PrintStream(new FileOutputStream("GameOfLifeOutput.txt")); // open a new PrintStream text file output object
      ...
      for(i=0;i<=gensIn;i++){                       // itterate through generations
        ...
        if(writeFlag==true){
          imObj.writeImage("OutputImages",("gameOfLifeImage"+i+".png")); // call a member function of the imObj object to output an image                                              
        }     
