...
import ca_cells.GOL2DCell;         // this class uses the lower level cell class to make an array of cell objects 


public class GOL2DGrid{
 
  //***************************************************************************************
  // Constructor - this initialises an array of cells
  //***************************************************************************************

  public GOL2DGrid(int xLen, int yLen) {    // constructor has cell array dimms passed in
                                            // in then initialises all private variables 

    size2D[0]=xLen;  // GOL2DGrid object stores x dimension of cell array
    size2D[1]=yLen;  // GOL2DGrid object stores y dimension of cell array
    ...
    cells= new GOL2DCell[size2D[0]][size2D[1]];     // initialise the array of cell objects
    for(i=0;i<size2D[0];i++){                       
      for(j=0;j<size2D[1];j++)  { 
        cells[i][j]=new GOL2DCell();                // call the constructor for each instance of the cell class
      }  
    }
  ...
  private GOL2DCell cells[][];            // declare the array of cells (of type GOL2DCell-previously imported)
  ...
  boolean[][] nMap = new boolean[size2D[0]+2][size2D[1]+2];   // a local array of states - this is 2 bigger than the cell array
    ...
    // for wrapped (toriodal) BCs... currently commented out.....
    for(i=0;i<size2D[0];i++){ 
